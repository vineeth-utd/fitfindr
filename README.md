...
FitFindr is a multi-tool AI agent that helps users discover secondhand clothing items, generate outfit recommendations, and create shareable fit cards. The agent combines retrieval from a thrift listings dataset with LLM-powered outfit generation and social-media-style caption creation, while maintaining state across multiple tool calls within a single interaction.

The project demonstrates multi-tool orchestration, planning-loop-based decision making, session state management, and graceful error handling in an end-to-end AI agent workflow.
...
## Demo Screenshot and Interface Overview

FitFindr provides a simple Gradio interface where users can search for secondhand clothing items and receive styling recommendations.

The interface consists of:

* **Query Input** – accepts natural language requests such as *"vintage graphic tee under $30"*.
* **Wardrobe Selector** – allows the user to choose between a predefined example wardrobe and an empty wardrobe representing a new user.
* **Top Listing Found** – displays the highest-ranked listing returned by `search_listings()`.
* **Outfit Idea** – displays the recommendation generated by `suggest_outfit()`.
* **Your Fit Card** – displays the social-media-style caption generated by `create_fit_card()`.

The screenshot below shows a successful end-to-end interaction where the agent retrieves a matching listing, generates an outfit recommendation using the selected wardrobe, and creates a shareable fit card.

![FitFindr Demo](images/fitfindr-demo.png)
...
**Purpose:**
Retrieves and ranks secondhand clothing listings from a mock thrift marketplace dataset based on the user's search criteria, budget, and size preferences.
...
The workflow stopped immediately and did not call `suggest_outfit()` or `create_fit_card()`.
